...
#const BOUNDS := Vector3(30.0, 30.0, 30.0)
#
#func lorenz_attractor(p: Vector3) -> Vector3:
	#const dt: float = 0.01
	#const SIGMA: float = 10.0
	#const RO: float = 28.0
	#const BETA: float = 2.666
#
	#return Vector3(
	...
	#)
#
#func generate_lorenz_path() -> PackedVector3Array:
	#var current := Vector3(0.1, 0.0, 0.0)
	#var path: PackedVector3Array = []
	#
	#for i in 200:
		#current = lorenz_attractor(current)
		#path.append(current * 0.1)
	#
	#return path
#
#func distance_to_path(pos: Vector3, path: PackedVector3Array) -> float:
	#var min_dist := 1000.0
	#
	#for i in range(path.size() - 1):
		#var seg_dist := distance_point_to_segment(pos, path[i], path[i + 1])
		#min_dist = min(min_dist, seg_dist)
	#
	#return min_dist
#
#func distance_point_to_segment(p: Vector3, a: Vector3, b: Vector3) -> float:
	#var ab := b - a
	#var ap := p - a
...
#
#func create_distance_field_texture() -> ImageTexture3D:
	#var path := generate_lorenz_path()
		...
		#for y in TEXTURE_SIZE:
			#for x in TEXTURE_SIZE:
				#var world_pos := Vector3(
					#(float(x) / TEXTURE_SIZE - 0.5) * BOUNDS.x,
					#(float(y) / TEXTURE_SIZE - 0.5) * BOUNDS.y,
					#(float(z) / TEXTURE_SIZE - 0.5) * BOUNDS.z
				#)
				#
				#var dist := distance_to_path(world_pos, path)
				#data.append(dist)
	#
	...
	#
	#for z in TEXTURE_SIZE:
		#var slice := Image.create(TEXTURE_SIZE, TEXTURE_SIZE, false, Image.FORMAT_RF)
		#for y in TEXTURE_SIZE:
			#for x in TEXTURE_SIZE:
				#var idx := z * TEXTURE_SIZE * TEXTURE_SIZE + y * TEXTURE_SIZE + x
				#var dist := data[idx]
				#slice.set_pixel(x, y, Color(0.686, 0.898, 0.996, 1.0))
		#images.append(slice)
	#
	#texture.create(Image.FORMAT_RF, TEXTURE_SIZE, TEXTURE_SIZE, TEXTURE_SIZE, false, images)
	#return texture
#
#func lorenz() -> void:
	#var distance_texture := create_distance_field_texture()
	#var bounds_uniform := Vector3(BOUNDS.x, BOUNDS.y, BOUNDS.z)
	#
	#%Fractal.material_override.set_shader_parameter('lorenz_distance_field', distance_texture)
	#%Fractal.material_override.set_shader_parameter('field_bounds', bounds_uniform)
...
func update_fractal_code(current_formulas: Array[int]) -> void:
	...
	if '// -@MultiDIFS':
		...
		for formula in (formulas_sorted as Array[Dictionary]):
			if formula['type'] == 'difs' or formula['type'] == 'primitive':
				var function: String = 'smax' if formula['type'] == 'difs' else 'smin'
				single_difs_code += '//de = ' + function + '(de, ' + formula['id'] + '_sdf(original_z).x, difs_smoothing); // -@' + str(formula['index']) + '\n'
	...
	if '// -@SingleFormulaCheck':
		...
		var single_formula_check_code := 'if ({check})'.replace('{check}', check_code)
		shader_code = shader_code.replace('// -@SingleFormulaCheck', single_formula_check_code)
